/* Usa um reduce() pra encadear uma quantidade indefinida de filtros em um array de objetos.
...
const handleFilter = (arrayOfObjects, method) => {
  ...
  return filters.reduce((acc, currFilter, index) => acc.filter((object) => { /* #2 */
    ...
    default: {
      if (!object[key[index]]) return null; /* #4 */
...
const arrayExample = [
  {name:'Caio', age: 25, city: 'São Paulo', genre: 'male'},
  {name: 'Petherson', age: 16, city: 'São Joaquim', genre: 'male'},
  {name: 'Lúcio', age: 34, city: 'Salvador', genre: 'male'},
  {name: 'Pablo', age: 27, city: 'Uberlândia', genre: 'other'},
  {name: 'Rodolfo', age: 19, city: 'Belo Horizonte', genre: 'male'},
  {name: 'Joarez', age: 46, city: 'São Paulo', genre: 'male'},
  {name: 'Helena', age: 32, city: 'Belo Horizonte', genre: 'female'},
];

console.log(handleFilter(arrayExample, {age: '<40', city: 'São'}));
console.log(handleFilter(arrayExample, {genre: 'female'}));
